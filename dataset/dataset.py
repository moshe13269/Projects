...
class Dataset:
    ...
    def __init__(self, path2load, type_files, dataset_names, data_split=False, labels=False):
        ...
        if self.data_split:
            ...
            for path in path2load:
                train_data = [join(path, 'data', file)
                              for file in listdir(join(path, 'data'))
                              if isfile(join(path, 'data', file)) and
                              os.path.splitext(file)[-1].lower()[1:] == self.type_files and
                              'label' not in join(path,  'data', file)]
                wav_files_train = wav_files_train + train_data

                test_data = [join(path, 'data', file)
                             for file in listdir(join(path, 'data'))
                             if isfile(join(path, 'data', file)) and
                             os.path.splitext(file)[-1].lower()[1:] == self.type_files and
                             'label' not in join(path, 'data', file)]
