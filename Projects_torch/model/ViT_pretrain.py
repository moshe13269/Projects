...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        ...
        if mask is not None:
            attn_scores = attn_scores.masked_fill(mask.cuda(), -1e10)  # (mask.cuda() == 0, -1e9)
    ...
    def forward(self, sequences, mask=None):
        ...
        for sequence in sequences:
            ...
            for head in range(self.n_heads):
                ...
                if mask is not None:
                    attention = self.softmax((q @ k.T / (self.d_head ** 0.5)) + mask)
                else:
...
class ViTBlock(nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, (list, tuple)):
            stft, mask = x
            out = stft + self.mhsa(self.norm1(stft), mask)
        else:
...
if __name__ == "__main__":
    vit = ViTPretrain(chw=(256, 128, 256), n_patches=7, n_blocks=12, hidden_d=512, n_heads=8, out_d=10, input_d=128)
