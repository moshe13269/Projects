...
def move_to(obj, device = torch.device("cpu")):
...
class SynthModules:
    ...
    def low_pass(self, input_signal, cutoff_freq, q=0.707, index=0):
        ...
        if cutoff_freq == 0:
            ...
            filtered_waveform_new = julius.lowpass_filter(input_signal, cutoff_freq / SAMPLE_RATE) #julius.lowpass_filter_new(input_signal, cutoff_freq / SAMPLE_RATE)
    ...
    def high_pass(self, input_signal, cutoff_freq, q=0.707, index=0):
        ...
        if cutoff_freq == 0:
            ...
            filtered_waveform_new = julius.highpass_filter(input_signal, cutoff_freq / SAMPLE_RATE) #julius.highpass_filter_new(input_signal, cutoff_freq / SAMPLE_RATE)
            return filtered_waveform_new
